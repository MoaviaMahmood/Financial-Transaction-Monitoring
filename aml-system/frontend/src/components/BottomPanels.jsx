...
export function GeoPanel({ data = [], onCountryClick }) {
    const max = data.length > 0 ? Math.max(...data.map((d) => d.v)) : 1;
...
export function RulesPanel({ data = [] }) {
                    ...
                    data.slice(0, 8).map((r) => {
                        ...
                        return (
                            <div key={r.label} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
                                <div style={{ flex: 1, fontSize: 11 }}>{r.label}</div>
                                ...
                                <div style={{ fontFamily: "'Orbitron',sans-serif", fontWeight: 700, fontSize: 11, width: 26, textAlign: "right", color: C.accent }}>{r.value}</div>
                            </div>
                        );
                    })
                )}
